...
function ensureLocalStore() {
  if (!fs.existsSync(LOCAL_DB_FILE)) {
    ...
    try {
      ...
      // Ignore write errors on read-only file systems like Vercel
    }
  } else {
    try {
...
export class TursoClient {
  ...
  async getEntries() {
    if (this.isTursoConfigured()) {
      try {
        ...
        console.warn('Turso fetch failed, attempting local fallback:', err.message);
      }
    }
    ...
    try {
      ...
      console.warn('Local store read warning:', err.message);
    }
    return [];
  }

  async saveEntry(entry) {
    // 1. If Turso is configured, save to Turso Cloud DB
    if (this.isTursoConfigured()) {
      await this.initTables();
      const sql = `
    ...
    // 2. Try saving to local store (ignored on Vercel read-only filesystem)
    try {
      ensureLocalStore();
      if (fs.existsSync(LOCAL_DB_FILE)) {
        const data = JSON.parse(fs.readFileSync(LOCAL_DB_FILE, 'utf-8'));
      ...
      if (!this.isTursoConfigured()) {
        throw new Error('Database is read-only on serverless deployment. Please set TURSO_DATABASE_URL and TURSO_AUTH_TOKEN in Environment Variables.');
      }
    }
  ...
  async deleteEntry(id) {
    // 1. Delete from Turso if configured
    if (this.isTursoConfigured()) {
      await this.executeSql('DELETE FROM entries WHERE id = ?', [id]);
    }

    // 2. Try deleting locally (ignored on Vercel read-only filesystem)
    try {
      ensureLocalStore();
      if (fs.existsSync(LOCAL_DB_FILE)) {
        const data = JSON.parse(fs.readFileSync(LOCAL_DB_FILE, 'utf-8'));
  ...
  // --- SAVED WORDS (SCRATCHPAD) METHODS ---
  async getSavedWords() {
    if (this.isTursoConfigured()) {
      try {
        await this.initTables();
        const rows = await this.executeSql('SELECT * FROM saved_words ORDER BY created_at DESC');
        return rows.map(r => ({
          id: r.id,
          word: r.word,
          notes: r.notes || '',
        ...
        console.warn('Turso fetch saved_words failed, attempting local fallback:', err.message);
      }
    }
    ...
    try {
      ...
      console.warn('Local store read savedWords warning:', err.message);
    }
    return [];
  }

  async saveSavedWord(savedWord) {
    if (this.isTursoConfigured()) {
      await this.initTables();
      const sql = `
        INSERT INTO saved_words (id, word, notes, created_at)
        VALUES (?, ?, ?, ?)
        ON CONFLICT(id) DO UPDATE SET
          word=excluded.word,
          notes=excluded.notes;
      ...
      await this.executeSql(sql, [
        ...
        savedWord.word,
        savedWord.notes || '',
        savedWord.createdAt || new Date().toISOString()
      ]);
    }
    ...
    try {
      ...
      if (fs.existsSync(LOCAL_DB_FILE)) {
        const data = JSON.parse(fs.readFileSync(LOCAL_DB_FILE, 'utf-8'));
        if (!Array.isArray(data.savedWords)) data.savedWords = [];
        const idx = data.savedWords.findIndex(w => w.id === savedWord.id || w.word.toLowerCase() === savedWord.word.toLowerCase());

        if (idx >= 0) {
          data.savedWords[idx] = { ...data.savedWords[idx], ...savedWord };
        } else {
          data.savedWords.unshift(savedWord);
        }
        fs.writeFileSync(LOCAL_DB_FILE, JSON.stringify(data, null, 2), 'utf-8');
      }
    } catch (err) {
      if (!this.isTursoConfigured()) {
        throw new Error('Database is read-only on serverless deployment.');
      }
    }
  ...
  async deleteSavedWord(id) {
    if (this.isTursoConfigured()) {
      await this.executeSql('DELETE FROM saved_words WHERE id = ?', [id]);
    }
    ...
    try {
      ...
      if (fs.existsSync(LOCAL_DB_FILE)) {
        const data = JSON.parse(fs.readFileSync(LOCAL_DB_FILE, 'utf-8'));
        if (Array.isArray(data.savedWords)) {
          data.savedWords = data.savedWords.filter(w => w.id !== id && w.word.toLowerCase() !== id.toLowerCase());
          fs.writeFileSync(LOCAL_DB_FILE, JSON.stringify(data, null, 2), 'utf-8');
        }
      }
    } catch (err) {
